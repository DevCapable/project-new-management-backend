...
@if($project && $currentWorkspace && $task)
    <form class="" method="post" action="@auth('web'){{ route('admin-tasks-update',[$currentWorkspace->slug,$project->project_id,$task->id]) }}@elseauth{{ route('client-tasks-update',[$currentWorkspace->slug,$project->project_id,$task->id]) }}@endauth">
        @csrf
...
{{--                        {{$projects->name}}--}}
                        @foreach($projects as $p)
                            <option value="{{$task->project_id}}" @if($task->project_id == $p->id) selected @endif>{{$p->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-md-12">
                    ...
                    <select class="multi-select" multiple="multiple" id="assign_to" name="assign_to[]" required>
                        @foreach($users as $u)
                            <option @if(in_array($u->id,$task->assign_to)) selected @endif value="{{$u->email}}">{{$u->name}} - {{$u->email}}</option>
                        @endforeach
                    </select>
                </div>
                @endif
