...
$('#dropzonewidget')
    .dropzone({
{{--url: "@auth('client') {{route('document_upload',$currentWorkspace->slug)}}--}}
    {{--    @elseauth('web') {{route('admin-document-upload',$currentWorkspace->slug)}}@endauth",--}}
        url: "{{route('document_upload',$currentWorkspace->slug)}}",
            headers: {
            'x-csrf-token': _token,
        },
        params: {
