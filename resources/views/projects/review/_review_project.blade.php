<form class="" method="post" action="{{ route('update-project-review',[$currentWorkspace->slug,$project->project_id]) }}">
    @csrf
            ...
            <div class="form-group col-md-12">
                ...
                <select id="status" name="status" class="form-control select2">
                    ...
                    <option value="NotSubmitted"
                            @if($project->status == 'NotSubmitted') selected @endif>{{ __('NotSubmitted') }}</option>
                    <option value="UnderReview"
                            @if($project->status == 'UnderReview') selected @endif>{{ __('UnderReview') }}</option>
                    <option value="Processing"
                            @if($project->status == 'Processing') selected @endif>{{ __('Processing') }}</option>
                    <option value="Finished"
                            @if($project->status == 'Finished') selected @endif>{{ __('Finished') }}</option>
                    <option value="PENDING"
                            @if($project->status == 'PENDING') selected @endif>{{ __('PENDING') }}</option>
