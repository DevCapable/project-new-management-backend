@if($project  && $task)
    <form class="" method="post"
          action="{{ route('admin-client-tasks-update',[$currentWorkspace->slug,$project->project_id,$task->id]) }}">
        @csrf
                    ...
                    <select class="form-control form-control-light select2" name="project_id" required>
                        @foreach($projects as $p)
                            <option value="{{$p->project_id}}"
                                    @if($task->project_id == $p->project_id) selected @endif>{{$p->name}}</option>
                        @endforeach
                    </select>
                </div>
...
{{--                                    @if($task->milestone_id == $milestone->id) selected @endif>{{$milestone->title}}</option>--}}
{{--                        @endforeach--}}
{{--                    </select>--}}
{{--                </div>--}}
                <div class="form-group col-md-8">
