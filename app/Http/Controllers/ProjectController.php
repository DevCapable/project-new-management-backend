...
//        dd($projectID);
    ...
    public function taskStore(Request $request, $slug, $projectID)
    {
        ...
        if ($project) {
            ...
            if ($stage) {$post['milestone_id'] = !empty($request->milestone_id) ? $request->milestone_id : 0;
    ...
    public function allTasks($slug)
    {
        ...
        if ($userObj->getGuard() == 'client') {
            ...
            $projects = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $userObj->id)->where('projects.workspace', '=', $currentWorkspace->id)->get();
        }
        $stages = Stage::where('workspace_id', '=', $currentWorkspace->id)->orderBy('order')->get();
