...
class AdminClientProjectsController extends Controller
{
...
//        dd($currentWorkspace->id);
        ...
        $projects = Project::select('projects.*')->join('client_projects', 'projects.project_id', '=', 'client_projects.project_id')->get();

        return view('admin.clients.projects.index', compact('currentWorkspace', 'projects'));
    }
    ...
    public function projectUnderReview($slug)
    {
        ...
        $projects = Project::select('projects.*')->join('client_projects', 'projects.project_id', '=', 'client_projects.project_id')
            ->where('projects.status', '=', get_projects_status(2))->get();
        return view('admin.clients.projects.index', compact('currentWorkspace', 'projects'));
    ...
    public function tracker($slug, $id)
    {
        ...
        if (isset($project) && $project != null) {
            return view('admin.clients.projects.tracker', compact('currentWorkspace', 'treckers', 'id', 'project'));
        } else {
            return redirect()->back()->with('error', __('Tracker Not Found.'));
        }
    }

    public function store($slug, \Illuminate\Http\Request $request)
    {
        ...
        $post = $request->only('project_name', 'project_description', 'budget', 'project_id');

        $post['start_date'] = date('Y-m-d');
        $post['end_date'] = date('Y-m-d');

        $post['name'] = $post['project_name'];
        ...
        $objProject = Project::create($post, ['description' => $post['project_description']]);
        ...
        return redirect()->route('client-projects-index', $currentWorkspace->slug)->with('success', __('Project Created Successfully!') . ((isset($smtp_error)) ? ' <br> <span class="text-danger">' . $smtp_error . '</span>' : ''));
    }


    public function getReview($slug, $projectID)
    {
//
        $objUser = Auth::user();
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('client_projects', 'projects.project_id', '=', 'client_projects.project_id')->where('projects.project_id', '=', $projectID)->first();
        $users = User::all();
        return view('admin.clients.review._review_project', compact('currentWorkspace', 'project', 'users'));
    }

    public function updateReview(Request $request, $slug, $projectID)
    {
//        $request->validate(
//            [
//                'name' => 'required',
...
//        );
        ...
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('client_projects', 'projects.project_id', '=', 'client_projects.project_id')->where('projects.project_id', '=', $projectID)->first();
        $project_data = Project::where('project_id', $projectID)->first();
        if ($request->proj_fee == 'paid') {

            $project_data->update(['is_paid' => 1]);
            $payment_info = new ProjectInvoicePayments();
            $payment_info->currency = $request->currency;
            $payment_info->amount_paid = $request->amount_required;
            $payment_info->txn_id = generate_code('CLT-PRJ');
            $payment_info->payment_type = 'CLIENT-PROJECT';
            $payment_info->payment_status = 'paid';
            $payment_info->payment_by = 'admin' . '|' . $objUser->email;
            $payment_info->project_id = $projectID;
            $payment_info->save();

            $clientPaymentInvoice = ProjectInvoicePayments::where('project_id', $projectID)->first();
            ActivityLog::create(
                [
                    'user_id' => \Auth::user()->id,
                    'user_type' => get_class(\Auth::user()),
                    'project_id' => $projectID,
                    'log_type' => 'Client Project fee Paid by ' . $objUser->email,
                    'remark' => json_encode(['title' => 'CLIENT-PROJECT']),
                ]
            );
            try {
                Mail::to($project->createrClient->email)->send(new SendClientProjectRecieptNotification($project->createrClient, $project, $clientPaymentInvoice));
            } catch (\Exception $e) {
                throw  $e;
                $smtp_error = __('E-Mail has been not sent due to SMTP configuration');
            }

        } elseif ($request->proj_fee == 'not_paid') {
            $project->update(['payment_links' => '<a href="' . route('make-payment', [$request->amount_required, $currentWorkspace->slug]) . '"><span  class="text-white">Click to make payment</span>
</a>']);
//            dd($project);
            try {
                Mail::to($project->createrClient->email)->send(new SendClientProjectPaymentNotification($project->createrClient, $project));
            } catch (\Exception $e) {
                throw  $e;
                $smtp_error = __('E-Mail has been not sent due to SMTP configuration');
            }
        ...
        $project->update($request->only('name', 'description', 'status', 'amount_required', 'start_date', 'end_date'));

        return redirect()->back()->with('success', __('Project Reviewd Successfully!'));
    }
...
//            dd($Data);
    ...
    {
        return view('admin.clients.projects.import', compact("slug"));
    }
    ...
    public function inviteUser(User $user, Project $project, $permission)
    {
        ...
        if (!$is_assigned) {
            ...
            try
            {
                ...
                $smtp_error = __('E-Mail has been not sent due to SMTP configuration,or error in network connection');
        ...
        if (!$is_invited) {
            ...
            if ($permission != 'Owner') {
                try
                {
                    ...
                    $smtp_error = __('E-Mail has been not sent due to SMTP configuration, or error in service connections');
                    return redirect()->back()->with('error',$smtp_error);
                }
    ...
    public function userPermission($slug, $project_id, $user_id)
    {
        ...
        return view('admin.clients.projects.user_permission', compact('currentWorkspace', 'project', 'user', 'permissions'));
    }
...
//        $task = Project::select('projects.*')->join('tasks', 'projects.id', '=', 'client_projects.project_id')->where('client_projects.client_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.id', '=', $projectID)->first();

        $route = 'client-comment';
        $daysleft = round((((strtotime($project->end_date) - strtotime(date('Y-m-d'))) / 24) / 60) / 60);


        return view('admin.clients.projects.show', compact('currentWorkspace', 'project', 'chartData', 'daysleft', 'route'));
    }
...
//        dd($objUser);
    ...
    public function create($slug, $id = null)
    {
        ...
        if (!empty($id)) {
            ...
            dd($app);
            return view('admin.clients.projects.create', compact('currentWorkspace', 'app'));
        }

//        $app = ClientProject::select('client_projects.*')->join('tasks', 'client_projects.id', '=', 'tasks.id')->where('client_projects.client_id', '=', $objUser->id)->get();
    ...
    public function popup($slug, $projectID)
    {
        $objUser = Auth::user();
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('projects.project_id', '=', $projectID)->first();
        $users = User::all();
        return view('admin.clients.projects.invite', compact('currentWorkspace', 'project','users'));
    }
    ...
    public function sharePopup($slug, $projectID)
    {
        ...
        return view('admin.clients.projects.share', compact('currentWorkspace', 'project'));
    }
    ...
    public function update(Request $request, $slug, $projectID)
    {
        ...
        $project->update($request->only('name', 'budget', 'start_date', 'end_date', '', ''));
...
//                $task->orderBy('order');
        ...
        return view('admin.clients.projects.taskboard', compact('currentWorkspace', 'project', 'stages', 'statusClass'));
    }
    ...
    public function taskCreate($slug, $projectID)
    {
        ...
        return view('admin.clients.projects.taskCreate', compact('currentWorkspace', 'project', 'projects', 'users'));
    }
...
//                    Utility::sendNotification('task_assign', $currentWorkspace, $request->assign_to, $task);
    ...
    public function taskReview($slug, $projectID, $taskId)
    {
//        dd($taskId);
        $objUser = Auth::user();
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        ...
        $project = Project::select('projects.*')->where('projects.project_id', '=', $projectID)->first();
        $projects = Project::select('projects.*')->join('client_projects', 'client_projects.project_id', '=', 'projects.project_id')->get();
...
//        $task->assign_to = explode(",", $task->assign_to);
        ...
        return view('admin.clients.projects.taskReview', compact('currentWorkspace', 'project', 'projects', 'users', 'task'));
...
//                'assign_to' => 'required',
        ...
        $project = Project::where('projects.project_id', '=', $projectID)->first();
//dd($project);
        if ($project) {
            $post = $request->all();
//            dd($post['project_id']);
    ...
    public function taskShow($slug, $projectID, $taskID)
    {
        ...
        return view('admin.clients.projects.taskShow', compact('currentWorkspace', 'task', 'clientID'));
    }
...
//        $project = Project::find($projectID);
//
//        return view('admin.clients.projects.milestone', compact('currentWorkspace', 'project'));
//    }
...
//
//        return view('admin.clients.projects.milestoneEdit', compact('currentWorkspace', 'milestone'));
//    }
...
//        $milestone = Milestone::find($milestoneID);
//
//        return view('admin.clients.projects.milestoneShow', compact('currentWorkspace', 'milestone'));
//    }
    ...
    public function addTask($slug, $projectID)
    {
        ...
        return view('admin.clients.projects.task._create_task', compact('currentWorkspace', 'project', 'info'));
    }
...
////                    dd($new_task->start_date);
//                $new_task->due_date = format_date($task['due_date'][$count], 'd/m/y');
//
//                $new_task->priority = $task['priority'][$count];
//
...
//    }
    ...
    public function timesheet($slug)
    {
        ...
        return view('admin.clients.projects.timesheet', compact('currentWorkspace', 'timesheets', 'project_id'));
    ...
    public function timesheetCreate($slug)
    {
        ...
        return view('admin.clients.projects.timesheetCreate', compact('currentWorkspace', 'projects'));
    }
    ...
    public function timesheetEdit($slug, $timesheetID)
    {
        ...
        return view('admin.clients.projects.timesheetEdit', compact('currentWorkspace', 'timesheet', 'projects'));
    }
    ...
    public function clientPermission($slug, $project_id, $client_id)
    {
        ...
        return view('admin.clients.projects.client_permission', compact('currentWorkspace', 'project', 'client', 'permissions'));
    }
    ...
    public function bugReport($slug, $project_id)
    {
        ...
        if ($project && (isset($permissions) && in_array('show bug report', $permissions)) || (isset($currentWorkspace) && $currentWorkspace->permission == 'Owner')) {
            ...
            return view('admin.clients.projects.bug_report', compact('currentWorkspace', 'project', 'stages', 'statusClass'));
        } else {
    ...
    public function bugReportCreate($slug, $project_id)
    {
        ...
        return view('admin.clients.projects.bug_report_create', compact('currentWorkspace', 'project', 'users', 'arrStatus'));
    }
    ...
    public function bugReportEdit($slug, $project_id, $bug_id)
    {
        ...
        return view('admin.clients.projects.bug_report_edit', compact('currentWorkspace', 'project', 'users', 'bug', 'arrStatus'));
    }
    ...
    public function bugReportShow($slug, $project_id, $bug_id)
    {
        ...
        return view('admin.clients.projects.bug_report_show', compact('currentWorkspace', 'bug', 'clientID'));
    }
    ...
    public function allTasks($slug)
    {
        ...
        return view('admin.clients.projects.tasks', compact('currentWorkspace', 'projects', 'users', 'stages'));
    }
...
//            )->join("user_projects", "tasks.project_id", "=", "user_projects.project_id")->join("projects", "projects.id", "=", "user_projects.project_id")->join("stages", "stages.id", "=", "tasks.status")->where("user_id", "=", $userObj->id)->where('projects.workspace', '=', $currentWorkspace->id);
    ...
    public function gantt($slug, $projectID, $duration = 'Week')
    {
        ...
        return view('admin.clients.projects.gantt', compact('currentWorkspace', 'project', 'tasks', 'duration', 'is_client'));
    }
    ...
    public function projectsTimesheet(Request $request, $slug, $project_id = 0)
    {
        ...
        return view('admin.clients.projects.timesheet', compact('currentWorkspace', 'project_id'));
    }
    ...
    public function projectTimesheetCreate(Request $request, $slug, $project_id)
    {
        ...
        if ($project_id) {
            ...
            if (!empty($project) && count($project) > 0) {
                ...
                return view('admin.clients.projects.timesheet-create', compact('currentWorkspace', 'parseArray'));
            }
        } else {
            $projects = $projects->get();

            return view('admin.clients.projects.timesheet-create', compact('currentWorkspace', 'projects', 'project_id', 'selected_date'));
        }
    }
    ...
    public function projectTimesheetEdit(Request $request, $slug, $timesheet_id, $project_id)
    {
        ...
        if ($timesheet) {
            ...
            if (!empty($project) && count($project) > 0) {
                ...
                return view('admin.clients.projects.timesheet-edit', compact('timesheet', 'currentWorkspace', 'parseArray', 'project_id'));
            }
        }
    }
