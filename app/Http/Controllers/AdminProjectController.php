...
use App\Http\Requests\Auth\adminTaskFormRequest;
...
use App\Models\Mail\SendUserProjectPaymentNotification;
use App\Models\Mail\SendUserProjectRecieptNotification;
...
class AdminProjectController extends Controller
{
    ...
    public function index($slug)
    {
        ...
        if ($objUser->type == 'admin') {
            $projects = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('projects.workspace', '=', $currentWorkspace->id)->get();
        } else {
    ...
    public function store($slug, Request $request)
    {
        ...
        if ($request->action == 'project_page') {
                return redirect()->route('admin-projects-index',[$currentWorkspace->slug,$check_project->project_id])->with(['success'=> __('Project not submitted yet, you can always go back to submit')]);
        }
        if ($request->action == 'submit') {
            $this->submit_project($request, $check_project);
        } else {
            ...
            foreach ($userList as $email) {
                ...
                if ($registerUsers) {
                    ...
                    try {
                        ...
                        $smtp_error = __('E-Mail has been not sent due to SMTP configuration,or bad network connection');
...
//                $this->inviteUser($post['action'], $registerUsers, $objProject, $permission);
            ...
            return redirect()->route('admin-show-project', [$currentWorkspace->slug, $objProject->project_id])->with(['success' => __('Project Created Successfully!') . ((isset($smtp_error)) ? ' <br> <span class="text-danger">' . $smtp_error . '</span>' : ''),
                'info' => $info
            ]);
        }
        if ($request->action == 'submit') {
            return redirect()->route('admin-projects-index', $currentWorkspace->slug)->with('success', __('Project Submitted Successfully!') . ((isset($smtp_error)) ? ' <br> <span class="text-danger">' . $smtp_error . '</span>' : ''));
        } else {
            $info = 'You can proceed on adding more tasks and submit';
            return redirect()->route('admin-show-project', [$currentWorkspace->slug, $check_project->project_id])->with(['success' => __('Project Created Successfully!') . ((isset($smtp_error)) ? ' <br> <span class="text-danger">' . $smtp_error . '</span>' : ''),
                'info' => $info
            ]);
        }
    }

    function submit_project($request, $check_project)
    {
...
//            ->where('projects.workspace', '=', $currentWorkspace->id)
        ...
        return view('projects.show', compact('currentWorkspace', 'project', 'chartData', 'daysleft', 'route', 'uploadedFiles', 'taskFiles'));
    }
...
//        dd($projectID);
        ...
        $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $projectID)->first();
        if (!$project) {
            $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $projectID)->first();

        }
        return view('projects.review._review_project', compact('currentWorkspace', 'project'));
    }
...
//        $users = User::select('users.*')->join('user_projects', 'user_projects.user_id', '=', 'users.id')->where('project_id', '=', $projectID)->get();
        $users = User::all();
        $route = 'show-admin-project';

        return view('projects.create', compact('currentWorkspace', 'users','route'));
    }
    ...
    public function userDelete($slug, $project_id, $user_id)
    {
        $objUser = Auth::user();
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $project_id)->first();
    ...
    public function update(Request $request, $slug, $projectID)
    {
        ...
        $objUser = Auth::user();
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.id', '=', $projectID)->first();
    ...
    public function updateReview(Request $request, $slug, $projectID)
    {
        ...
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $projectID)->first();
        if ($project == null && Auth::user()->type == 'admin') {
            $project = Project::select('projects.*')->join('user_projects', 'projects.project_id', '=', 'user_projects.project_id')->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $projectID)->first();
        }
        if ($request->proj_fee == 'paid') {
            $rules = ['amount_required' => 'required'];

            $validate = Validator::make($request->only('amount_required'), $rules);
            if ($validate->fails()) {
                return redirect()->back()->withErrors($validate)->with('error', __('check, you  forgot to fill amount required filed'));
            } else {
                $project->update(['is_paid' => 1]);
                ...
                $payment_info->txn_id = generate_code('PRJ');
                $payment_info->payment_type = 'PROJECT';
                ...
                ActivityLog::create(
                    [
                        ...
                        'user_type' => get_class(\Auth::user()),
                        'project_id' => $projectID,
                        'log_type' => 'Admin Project fee Paid by ' . $objUser->email,
            ...
            try {
                Mail::to($project->creater->email)->send(new SendUserProjectRecieptNotification($project->creater, $project, $clientPaymentInvoice));
            } catch (\Exception $e) {
                $smtp_error = __('E-Mail has been not sent due to SMTP configuration,or bad network connection');
                return redirect()->back()->with('error',$smtp_error);
            }
            ...
            try {
                Mail::to($project->creater->email)->send(new SendUserProjectPaymentNotification($project->creater, $project));
            } catch (\Exception $e) {
                $smtp_error = __('E-Mail has been not sent due to SMTP configuration,or bad network connection');
                return redirect()->back()->with('error',$smtp_error);
            }
...
dd($project);
    ...
    {
        $objUser = Auth::user();
        $userProject = Project::where('project_id',$projectID)->first();
...
//        $users = User::select('users.*')->join('user_projects', 'user_projects.user_id', '=', 'users.id')->where('project_id', '=', $projectID)->get();
        $users = User::all();
        $route = 'show-admin-project';
        return view('projects.task._create_task', compact('currentWorkspace', 'project', 'users','route'));
    }
...
//        $users = User::select('users.*')->join('user_projects', 'user_projects.user_id', '=', 'users.id')->where('project_id', '=', $projectID)->get();
    ...
    public function storeTask($slug, $projectID, adminTaskFormRequest $request)
    {
...
//                    dd($new_task->start_date);
                ...
                $new_task->save();
                $project->update(['status' => get_projects_status(2)]);
...
//        $project = Task::find($id);
    ...
    public function taskEdit($slug, $projectID, $taskId)
    {
        ...
        if ($objUser->getGuard() == 'client') {
            ...
            $project = Project::select('projects.*')->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $projectID)->first();
            $projects = Project::select('projects.*')->join('user_projects', 'user_projects.project_id', '=', 'projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->get();
        }
        $users = User::select('users.*')->join('user_projects', 'user_projects.user_id', '=', 'users.id')->where('project_id', '=', $projectID)->get();
        $task = Task::find($taskId);
    ...
    public function taskUpdate(Request $request, $slug, $projectID, $taskID)
    {
        ...
        $project = Project::select('projects.*')->join('user_projects', 'user_projects.project_id', '=', 'projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.project_id', '=', $request->project_id)->first();
        if ($project) {
            $post = $request->all();
//            $post['assign_to'] = implode(",", $request->assign_to);
            $task = Task::find($taskID);
...
//    }
    ...
    public function taskShow($slug, $projectID, $taskID)
    {
        ...
        return view('clients.projects.taskShow', compact('currentWorkspace', 'task', 'clientID'));
    }
    ...
    public function milestoneShow($slug, $milestoneID)
    {
        ...
        return view('clients.projects.milestoneShow', compact('currentWorkspace', 'milestone'));
    }
    ...
    public function fileUpload($slug, $projectID, Request $request)
    {
        $project = Project::where('project_id', $projectID)->first();
        $request->validate(['file' => 'required|mimes:zip,rar,jpeg,jpg,png,gif,svg,pdf,txt,doc,docx,application/octet-stream,audio/mpeg,mpga,mp3,wav|max:204800']);
        $file_name = $request->file->getClientOriginalName();
        $file_path = $project->project_id . "_" . md5(time()) . "_" . $request->file->getClientOriginalName();
    ...
    public function fileDownload($slug, $project_id, $file_id)
    {
    ...
    public function fileDelete($slug, $project_id, $file_id)
    {
        $project = Project::where('project_id', $project_id)->first();
