...
//            }
    ...
    public function inviteUser($slug, Request $request)
    {
        ...
        if(!$registerUsers)
        {
           ...
           $arrUser['curren t_workspace'] = $currentWorkspace->id;
    ...
    public function updatePaymentPolicy(Request $request){
        ...
        \App::setLocale($lang);
        $update = Client::where('id',$request->user_id)->first();
        $client = Client::where('verification_code', $update->verification_code)->first();

        $update->payment_policy = 1;
        $update->save();
        return view('auth._client_reg_payment', compact('lang', 'client'))->with('success', 'you just signed undertaking');
